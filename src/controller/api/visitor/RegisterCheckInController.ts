...
export const RegisterCheckInController = async (
  req: Request,
  res: Response
): Promise<e.Response> => {
  try {
    ...
    const visitorData: RegisterRequestModel = new RegisterRequestModel(
      req.body
    );
    ...
    while (true) {
      ...
      const visitorExists = await visitorLogRepository.existsByPinCode(
        pinCode,
        qrKey
      );
