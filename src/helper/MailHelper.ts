...
export const testSentEmail = (name: string, phoneNumber: string, email: string, date: string, qrKey: string): void => {

    const htmlFilePath = path.join(__dirname, '../template/email_approve_template.html');
    ...
    const data: { [key: string]: string } = {
        ...
        email: email,
        date: date,
        confirmUrl: process.env.HOST_NAME +'/v1/approve/' + qrKey
    };
